...
function generateStaticHTML(node) {
	...
	if (t.isJSXElement(node)) {
		...
		if (t.isJSXNamespacedName(node.openingElement.name)) {
			// Simple handling: use full tag for now, browser innerHTML handles common cases
			tagName = node.openingElement.name.namespace.name + ":" + tagName;
		}

		// TODO may this handle decorator or whatever?
...
function isStaticJSX(path, isChild) {
	...
	return path.get("children").every(childPath => {
		...
		if (t.isJSXText(child) || isStaticAttribute(child)) {
			return true;
		}
